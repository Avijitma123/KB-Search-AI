...
class KnowledgeBasedAgent:
  ...
  def choose_location(self):
    """This creates a set of unvisited locations using the unvisited method of the Agent class."""
    unvisited_locations = self.unvisited()
    
    """
    This creates a set of safe moves using the safe method of the Agent class, 
    and takes the intersection with the unvisited locations to get the safe moves that haven't been visited yet.
    """
    safe_moves = self.safe().intersection(unvisited_locations)
    
    """
    If there are safe moves available, the function selects the next location by taking the minimum location from the set of safe moves, 
    and prints a message to indicate that the agent is moving to that safe location.
    """
    if safe_moves:
        location = min(safe_moves)
        print('Moving to safe location', location)
    else:
        #If there are no safe moves available, the function creates a set of not-unsafe moves using the not_unsafe method of the Agent class, and takes the intersection with the unvisited locations to get the not-unsafe moves that haven't been visited yet.
        not_unsafe_moves = self.not_unsafe().intersection(unvisited_locations)
        if not_unsafe_moves:
            #If there are not-unsafe moves available, the function selects the next location by taking the minimum location from the set of not-unsafe moves, and prints a message to indicate that the agent is taking a risk by moving to that not-unsafe location.
            location = min(not_unsafe_moves)
            print('Taking a risk; moving to a not-unsafe location', location)
        else:
            #If there are no safe or not-unsafe moves available, the function prints a message to indicate that there is nowhere left to go, and raises a GameOver exception with a result of RESULT_GIVE_UP.
