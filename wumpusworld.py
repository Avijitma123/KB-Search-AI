...
class WumpusWorldAgent(inference.KnowledgeBasedAgent):
  ...
  """
    ...
    def safe(self):
        ...
        """
        # creates an empty set to store the safe locations.
        safe_pos = set()
        # This loops through all the locations in the environment, where self.size is the size of the environment.
        for x in range(1, self.size + 1):
            for y in range(1, self.size + 1):
                # This creates a string location that represents the current location using the format x_y.
                location = "" + str(x) + "_" + str(y)

                # Add current location spot to the safe_spots
                if inference.resolution(self.KB, logic.expr("L" + location)):
                    safe_pos.add((x, y))

                # Not a pit or not a wumpus at current location
                if inference.resolution(self.KB, logic.expr("~P" + location)) and inference.resolution(self.KB, logic.expr("~W" + location)):
                    safe_pos.add((x, y))

                # If no smell, and no breeze, then all neighbors are safe locations.
                no_smell = inference.resolution(self.KB, logic.expr("~S" + location))
                no_breeze = inference.resolution(self.KB, logic.expr("~B" + location))
                if (no_smell and no_breeze):
                    for n in get_neighbors(x, y, self.size):
                        """This checks if there is no smell and no breeze at the current location based on the knowledge base self.KB, 
                        and if so, adds all the neighbors of the current location to the safe_spots set."""
                        safe_pos.add(n)
  ...
  """
    ...
    def not_unsafe(self):
        """
        This defines a method called not_unsafe that determines the set of locations that the agent can't prove to be unsafe. 
        The method is a member of a class that represents an agent in the Wumpus World.
        """
        #This creates an empty set to store the not-unsafe locations.
        not_unsafe_pos = set()
        
        """
           This loops through all the locations in the environment, 
           where self.size is the size of the environment.
        """
        for x in range(1, self.size + 1):
            for y in range(1, self.size + 1):
                ...
                if not logic.resolution(self.KB, logic.expr("L" + location)):
                    not_unsafe_pos.add((x, y))

                if logic.resolution(self.KB, logic.expr("P" + location)) or logic.resolution(self.KB, logic.expr("W" + location)):
                    if not_unsafe_pos.__contains__((x, y)):
                        not_unsafe_pos.remove((x, y))

                # This checks if the current location is inferred to be a pit or a Wumpus based on the knowledge base self.KB, and 
                #if so, removes the location from the not_unsafe_spots set if it's already in there.
                no_smell = logic.resolution(self.KB, logic.expr("~S" + location))
                no_breeze = logic.resolution(self.KB, logic.expr("~B" + location))
                if (no_smell and no_breeze):
                    for n in get_neighbors(x, y, self.size):
                        
                        not_unsafe_pos.add(n)

        return not_unsafe_pos
